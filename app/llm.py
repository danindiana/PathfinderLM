...
def generate(prompt: str, model: Optional[str] = None) -> str:
    ...
    if provider == "ollama":
        return _generate_ollama(prompt, model or config.model_name, config.ollama_host)
    if provider == "openai":
        return _generate_openai(prompt, model or config.model_name)
    raise ValueError(f"Unknown LLM_PROVIDER: {config.llm_provider!r} (expected 'ollama' or 'openai')")


def _generate_ollama(prompt: str, model: str, host: str) -> str:
